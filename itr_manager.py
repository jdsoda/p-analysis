...
def normalize_date(value):
    ...
    for fmt in ('%d/%b/%Y','%d/%b/%y','%d/%m/%Y','%d/%m/%y','%d/%b/%Y %H:%M:%S','%d/%m/%Y %H:%M:%S','%d/%b/%Y %H:%M','%d/%m/%Y %H:%M'):
        try:return datetime.strptime(s,fmt).strftime('%Y-%m-%d')
        except ValueError:continue
    m=re.search(r'(\d{1,2})[/-]([A-Za-z]{3}|\d{1,2})[/-](20\d{2})',s)
    if m:
        try: return datetime.strptime(f'{int(m.group(1)):02d}/{m.group(2)}/{m.group(3)}','%d/%b/%Y').strftime('%Y-%m-%d')
        ...
        for page in pdf.pages:
            for table in (page.extract_tables() or []):
                for raw in table:
                    ...
                    date_idx=next((i for i,c in enumerate(cells) if re.search(r'\b\d{1,2}[/-](?:[A-Za-z]{3}|\d{1,2})[/-](?:20)?\d{2}\b',c)),None)
                    if date_idx is None:continue
                    txn_date=normalize_date(cells[date_idx]); numeric=[(i,clean_amount(c)) for i,c in enumerate(cells) if clean_amount(c) is not None]
                    if not numeric:continue
                    nums=[v for _,v in numeric[-3:]]; balance=nums[-1] if nums else None; debit=nums[-3] if len(nums)==3 else 0; credit=nums[-2] if len(nums)==3 else 0
                    if len(nums)==2: debit,credit=0,nums[0]
                    if len(nums)==1: debit,credit=0,nums[0]
                    narration=' '.join(c for i,c in enumerate(cells) if i!=date_idx and clean_amount(c) is None).strip()
                    ...
                    rows.append({'txn_date':txn_date,'narration':narration,'debit':round(abs(debit or 0),2),'credit':round(abs(credit or 0),2),'balance':balance,'channel':infer_channel(narration),'suggested_category':cat,'confidence':confidence})
    unique={}
    for r in rows: unique[(r['txn_date'],r['narration'],r['debit'],r['credit'],r['balance'])]=r
...
def new_year(client_id):
    ...
    try:
        cur=con.execute('INSERT INTO financial_years(client_id,financial_year,status,created_at,updated_at) VALUES(?,?,"projected",?,?)',(client_id,fy,now,now)); fyid=cur.lastrowid; ensure_defaults(con,client_id); prev_id=int(previous) if previous else None; create_values_from_previous(con,fyid,prev_id,client_id); con.commit()
...
def bank_import(fyid):
    ...
    if request.method=='POST':
        ...
        if not file or not file.filename.lower().endswith('.pdf'):
            flash('Please select a bank statement PDF.'); con.close(); return redirect(url_for('itr.bank_import',fyid=fyid))
        safe=secure_filename(file.filename); path=os.path.join(UPLOAD_DIR,f'{fyid}_{safe}'); file.save(path)
        try:
            rows=parse_bank_pdf(path,cash_mode); now=datetime.utcnow().isoformat()
            for r in rows:
                con.execute('INSERT OR IGNORE INTO bank_transactions(financial_year_id,txn_date,narration,debit,credit,balance,channel,suggested_category,confidence,final_category,source_file,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(fyid,r['txn_date'],r['narration'],r['debit'],r['credit'],r['balance'],r['channel'],r['suggested_category'],r['confidence'],r['suggested_category'],safe,now))
            con.execute('UPDATE financial_years SET bank_receipts_mode=?,bank_import_file=?,updated_at=? WHERE id=?',('CASH_AS_SALES' if cash_mode else 'REVIEW',safe,now,fyid)); con.commit(); flash(f'Imported {len(rows)} bank transactions. Please review classification before posting.')
