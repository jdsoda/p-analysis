...
UPLOAD_DIR=os.environ.get('ITR_UPLOAD_DIR',os.path.join(os.path.dirname(__file__),'itr_uploads')); os.makedirs(UPLOAD_DIR,exist_ok=True)
        ...
        for page in pdf.pages:
            for table in (page.extract_tables() or []):
                for raw in table:
                    ...
                    # This bank model is Date, Particulars, Chq/Ref, Debit, Credit, Balance.
                    if len(cells)>=7 and date_idx==1 and clean_amount(cells[4]) is not None and clean_amount(cells[5]) is not None and clean_amount(cells[6]) is not None:
                        ...
                        nums=[v for _,v in numeric[-3:]]; balance=nums[-1]; debit=nums[-3] if len(nums)==3 else 0; credit=nums[-2] if len(nums)==3 else (nums[0] if nums else 0)
                        narration=' '.join(c for i,c in enumerate(cells) if i!=date_idx and clean_amount(c) is None).strip(); txn_date=normalize_date(cells[date_idx])
                    ...
                    rows.append({'txn_date':txn_date,'narration':narration,'debit':round(abs(debit),2),'credit':round(abs(credit),2),'balance':balance,'channel':infer_channel(narration),'suggested_category':cat,'confidence':confidence})
