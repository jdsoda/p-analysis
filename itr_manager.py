...
FIELDS=[
 ('TRADING','Opening Stock'),('TRADING','Purchases'),('TRADING','Direct Expenses'),('TRADING','Sales'),('TRADING','Commission / Discounts'),('TRADING','Closing Stock'),('TRADING','Gross Profit'),
 ('P&L','Salary'),('P&L','Rent'),('P&L','Electricity Charges'),('P&L','Telephone Expenses'),('P&L','Repairs & Maintenance'),('P&L','Miscellaneous Expenses'),('P&L','Depreciation'),('P&L','Interest'),('P&L','Other Expenses'),('P&L','Net Profit'),
 ('BALANCE_SHEET','Capital Account'),('BALANCE_SHEET','Fixed Assets'),('BALANCE_SHEET','Depreciation'),('BALANCE_SHEET','Investments'),('BALANCE_SHEET','Loan'),('BALANCE_SHEET','Advance / Deposits & Advances'),('BALANCE_SHEET','Closing Stock'),('BALANCE_SHEET','Debtors'),('BALANCE_SHEET','Creditors'),('BALANCE_SHEET','Bank'),('BALANCE_SHEET','Cash'),('BALANCE_SHEET','Other Assets'),('BALANCE_SHEET','Other Liabilities')]
...
def set_value(con,fyid,field,amount,source='CALCULATED'):
 con.execute('UPDATE account_values SET amount=?,source_type=? WHERE financial_year_id=? AND field_key=?',(round(float(amount),2),source,fyid,field))

def calculate_statements(con,fyid):
 vals={r['field_key']:float(r['amount'] or 0) for r in get_values(con,fyid)}
 opening=vals.get('Opening Stock',0); purchases=vals.get('Purchases',0); direct=vals.get('Direct Expenses',0); sales=vals.get('Sales',0); closing=vals.get('Closing Stock',0); commission=vals.get('Commission / Discounts',0)
 gross=sales+commission-(opening+purchases+direct)+closing
 operating=sum(vals.get(k,0) for k in ('Salary','Rent','Electricity Charges','Telephone Expenses','Repairs & Maintenance','Miscellaneous Expenses','Depreciation','Interest','Other Expenses'))
 net=gross-operating
 set_value(con,fyid,'Gross Profit',gross,'CALCULATED')
 set_value(con,fyid,'Net Profit',net,'CALCULATED')
 set_value(con,fyid,'Closing Stock',closing,'SYNC_FROM_TRADING')
 return {'opening_stock':opening,'purchases':purchases,'direct_expenses':direct,'sales':sales,'closing_stock':closing,'commission':commission,'gross_profit':gross,'operating_expenses':operating,'net_profit':net}
...
def create_values_from_previous(con,fyid,previous_id,client_id):
 ...
 calculated={'Gross Profit','Net Profit'}
 for statement,field in FIELDS:
  p=prev.get(field); s=settings.get(field); base=float(p['amount']) if p else 0; pct=float(s['growth_percent']) if s else 10; method=s['growth_method'] if s else 'AUTO_GROWTH'; amount=base*(1+pct/100) if method=='AUTO_GROWTH' else base
  if field in calculated: amount=base
...
def parse_bank_pdf(path,cash_deposits_as_sales=False):
 ...
 return list({(r['txn_date'],r['narration'],r['debit'],r['credit'],r['balance']):r for r in rows}.values())
...
def year(fyid):
 ...
 if request.method=='POST':
  for k,v in request.form.items():
   ...
   field=k[4:]
   if field not in ('Gross Profit','Net Profit'): con.execute('UPDATE account_values SET amount=?,source_type="MANUAL" WHERE financial_year_id=? AND field_key=?',(amount,fyid,field))
  calculate_statements(con,fyid); con.execute('UPDATE financial_years SET status="actual",updated_at=? WHERE id=?',(datetime.utcnow().isoformat(),fyid)); con.commit(); flash('Trading A/c, Gross Profit and P&L recalculated.')
 else:
  calculate_statements(con,fyid); con.commit()
 vals=get_values(con,fyid); groups={}
 for r in vals:groups.setdefault(r['statement_type'],[]).append(r)
 calc=calculate_statements(con,fyid); con.commit(); con.close(); return render_template('itr_year.html',fy=fy,groups=groups,calc=calc)

@itr.route('/year/<int:fyid>/projection-settings',methods=['POST'])
def projection_settings(fyid):
 con=db(); fy=con.execute('SELECT * FROM financial_years WHERE id=?',(fyid,)).fetchone()
 if not fy:con.close(); return 'Financial year not found',404
 ensure_defaults(con,fy['client_id'])
 for field in [x[1] for x in FIELDS]:
  if f'growth_{field}' in request.form:
   try:pct=float(request.form.get(f'growth_{field}',10))
   except:pct=10
   method=request.form.get(f'method_{field}','AUTO_GROWTH')
   if method not in ('AUTO_GROWTH','CARRY_FORWARD'):method='AUTO_GROWTH'
   con.execute('UPDATE projection_settings SET growth_percent=?,growth_method=? WHERE client_id=? AND field_key=?',(pct,method,fy['client_id'],field))
 con.commit(); con.close(); flash('Projection settings saved.'); return redirect(url_for('itr.year',fyid=fyid))
...
def bank_import(fyid):
 ...
 if request.method=='POST':
  ...
  try:
   ...
   if start and end: rows=[r for r in rows if r['txn_date'] and start<=datetime.strptime(r['txn_date'],'%Y-%m-%d').date()<=end]
   ...
   con.execute('UPDATE financial_years SET bank_receipts_mode=?,bank_import_file=?,updated_at=? WHERE id=?',('CASH_AS_SALES' if cash_mode else 'REVIEW',safe,now,fyid)); con.commit(); flash(f'Imported {len(rows)} transactions for {fy["financial_year"]}. Review classification before posting.')
...
def save_bank_categories(fyid):
 ...
 con.commit(); con.close(); flash('Bank classifications saved. Nothing is posted until Apply Bank Totals.'); return redirect(url_for('itr.bank_import',fyid=fyid))
...
def apply_bank_totals(fyid):
 ...
 con.execute("UPDATE account_values SET amount=?,source_type='BANK_IMPORT' WHERE financial_year_id=? AND field_key='Sales'",(sales,fyid)); con.execute("UPDATE account_values SET amount=?,source_type='BANK_IMPORT' WHERE financial_year_id=? AND field_key='Purchases'",(purchases,fyid)); calculate_statements(con,fyid); con.execute('UPDATE financial_years SET updated_at=? WHERE id=?',(datetime.utcnow().isoformat(),fyid)); con.commit(); con.close(); flash(f'Applied Bank Review totals: Sales ₹{sales:,.2f}, Purchases ₹{purchases:,.2f}. Trading/P&L recalculated.'); return redirect(url_for('itr.year',fyid=fyid))
